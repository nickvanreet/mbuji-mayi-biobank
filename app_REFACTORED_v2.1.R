# MBUJI-MAYI BIOBANK DASHBOARD — REFACTORED v2.1
# =============================================================================
# Clean Overview + Dedicated Transport Tab
# Enhanced performance with caching and precomputed summaries
# =============================================================================
...
load_config <- function() {
  cfg <- if (file.exists("config.yml")) {
    ...
    list(
      ...
      app = list(
        title = "Mbuji-Mayi Biobank Dashboard", version = "2.1.0",
...
ui <- page_navbar(
  ...
  nav_panel(
    title = "Overview",
    # Compact KPI strip with tooltips
    layout_columns(
    ...
    layout_columns(
      ...
      card(
        ...
        checkboxInput("show_overlay", "Show full dataset overlay", value = TRUE),
        plotOutput("plot_age_sex_pyramid", height = 300)
      )
    ),
    ...
    layout_columns(
      ...
      card(card_header("Demographics"), tableOutput("table_demographics")),
      card(card_header("Case History"), tableOutput("table_case_history")),
      card(card_header("Geographic Coverage"), tableOutput("table_geography")),
      card(card_header("Top Sites"), tableOutput("table_top_sites"))
    )
  ),
...
server <- function(input, output, session) {
  ...
  output$plot_age_sex_pyramid <- renderPlot({
    ...
    p <- ggplot(plot_data, aes(x = age_group, y = count, fill = sex_label, alpha = dataset)) +
      ...
      labs(
        ...
        subtitle = if (low_sample_warning) "⚠️ Low sample size (<5)" else NULL
      ) +
      theme_minimal(base_size = 13) +
      theme(
  ...
  output$table_demographics <- renderTable({
    ...
    tibble(
      ...
      Value = c(
        ...
        sprintf("%.0f - %.0f", min(df$age_num, na.rm = TRUE), 
                max(df$age_num, na.rm = TRUE))
      )
    )
  }, striped = TRUE, hover = TRUE, bordered = TRUE)
  
  output$table_case_history <- renderTable({
    df <- filtered_data(); req(df, nrow(df) > 0)
    
    # Check if same person by combining ancien_cas and traite
    tibble(
      Metric = c("Previous cases", "Previously treated", "New patients"),
      Count = c(
        sum(df$ancien_cas == "Oui", na.rm = TRUE),
        sum(df$traite == "Oui", na.rm = TRUE),
        sum(df$ancien_cas == "Non" & df$traite == "Non", na.rm = TRUE)
      ),
      Percent = sprintf("%.1f%%", Count / nrow(df) * 100)
    )
  }, striped = TRUE, hover = TRUE, bordered = TRUE)
  ...
  output$table_geography <- renderTable({
    ...
    tibble(
      Metric = c("Provinces", "Health zones", "Fixed sites", "Mobile units"),
      Count = c(
        n_distinct(df$province, na.rm = TRUE),
        n_distinct(df$zone, na.rm = TRUE),
        n_distinct(structures_classified$structure[!structures_classified$is_mobile]),
        n_distinct(df$unit[!is.na(df$unit) & nzchar(df$unit)])
      )
    )
  }, striped = TRUE, hover = TRUE, bordered = TRUE)
