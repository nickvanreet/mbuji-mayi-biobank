...
# --- ICON FALLBACKS (Bootstrap → alias → FontAwesome) -----------------------
# Map domain names or missing Bootstrap icons to close equivalents.
.icon_alias <- c(
  microscope = "beaker",         # BS alias; FA fallback "microscope"
  pcr        = "dna",            # FA concept
  elisa      = "table",          # FA "table-cells" concept
  plate      = "grid-3x3-gap",
  freezer    = "snow",
  storage    = "archive",
  shipment   = "truck",
  qc         = "shield-check-fill",
  dna        = "dna",
  vial       = "beaker",
  sample     = "droplet-half",
  barcode    = "upc-scan",
...
.has_bs_icon <- function(name) {
  if (is.null(name) || !nzchar(name)) return(FALSE)
  !inherits(try(bsicons::bs_icon(name), silent = TRUE), "try-error")
}

# Shadow bs_icon safely: Bootstrap → alias (Bootstrap) → Font Awesome → fallback
bs_icon <- function(name, ...) {
  # 1) Try requested Bootstrap icon
  if (.has_bs_icon(name)) return(bsicons::bs_icon(name, ...))
  
  # 2) Safe alias lookup (no subscript error)
  alias_raw <- .icon_alias[as.character(name)]
  alt <- if (length(alias_raw) && !is.na(alias_raw)) unname(alias_raw) else NULL
  if (!is.null(alt) && .has_bs_icon(alt)) return(bsicons::bs_icon(alt, ...))
  
  # 3) Try Font Awesome with alias or original
  fa_try <- alt %||% name
  fa_tag <- try(shiny::icon(fa_try), silent = TRUE)
  if (!inherits(fa_tag, "try-error")) return(fa_tag)
...
load_config <- function() {
  cfg <- if (file.exists("config.yml")) {
    ...
    list(
      ...
      app = list(
        title = "Mbuji-Mayi Biobank Dashboard", version = "2.5.0",
...
source_if_exists("R/utils_parse_v2.R", local = TRUE)
source_if_exists("R/utils_join.R", local = TRUE)
source_if_exists("R/utils_data_quality.R", local = TRUE)  # New file for data quality functions
...
analyze_data_quality <- function(df_raw) {
  if (is.null(df_raw) || !nrow(df_raw)) {
    return(list(
      ...
      empty_rows = 0,
      valid_rows = 0,
      invalid_missing_barcode = 0,
      invalid_missing_labid = 0,
      invalid_missing_both = 0,
      duplicates = tibble(),
      barcode_conflicts = tibble(),
      column_completeness = tibble()
    ))
  }
  ...
  if (!barcode_col %in% names(df_raw)) df_raw[[barcode_col]] <- NA_character_
  if (!labid_col %in% names(df_raw)) df_raw[[labid_col]] <- NA_character_
  
  # Convert to character and clean
  df_raw <- df_raw %>%
    mutate(
      barcode_clean = trimws(as.character(!!sym(barcode_col))),
      labid_clean = trimws(as.character(!!sym(labid_col))),
      barcode_valid = !is.na(barcode_clean) & nzchar(barcode_clean) & barcode_clean != "NA",
      labid_valid = !is.na(labid_clean) & nzchar(labid_clean) & labid_clean != "NA",
      row_valid = barcode_valid & labid_valid
    )
  
  # Count different types of rows
  total_rows <- nrow(df_raw)
  valid_rows <- sum(df_raw$row_valid)
  empty_rows <- sum(!df_raw$barcode_valid & !df_raw$labid_valid)
  invalid_missing_barcode <- sum(!df_raw$barcode_valid & df_raw$labid_valid)
  invalid_missing_labid <- sum(df_raw$barcode_valid & !df_raw$labid_valid)
  invalid_missing_both <- sum(!df_raw$barcode_valid & !df_raw$labid_valid)
  
  # Find duplicates (among valid rows)
  duplicates <- df_raw %>%
    filter(row_valid) %>%
    group_by(barcode_clean, labid_clean) %>%
    filter(n() > 1) %>%
    summarise(
      count = n(),
      rows = paste(which(df_raw$barcode_clean == barcode_clean[1] & 
                           df_raw$labid_clean == labid_clean[1]), collapse = ", "),
      .groups = "drop"
    ) %>%
    rename(barcode = barcode_clean, lab_id = labid_clean)
  
  # Find barcode conflicts (same barcode, different lab_id)
  barcode_conflicts <- df_raw %>%
    filter(row_valid) %>%
    group_by(barcode_clean) %>%
    filter(n_distinct(labid_clean) > 1) %>%
    summarise(
      n_lab_ids = n_distinct(labid_clean),
      lab_ids = paste(unique(labid_clean), collapse = " | "),
      count = n(),
      .groups = "drop"
    ) %>%
    rename(barcode = barcode_clean)
  
  # Column completeness analysis
  column_completeness <- df_raw %>%
    filter(row_valid) %>%
    select(-barcode_clean, -labid_clean, -barcode_valid, -labid_valid, -row_valid) %>%
    summarise(across(everything(), 
                     ~sum(!is.na(.) & nzchar(trimws(as.character(.))) & . != "NA"))) %>%
    pivot_longer(everything(), names_to = "column", values_to = "non_empty") %>%
    mutate(
      total = valid_rows,
      percent = round(100 * non_empty / valid_rows, 1)
    ) %>%
  ...
  list(
    ...
    invalid_missing_barcode = invalid_missing_barcode,
    invalid_missing_labid = invalid_missing_labid,
    invalid_missing_both = invalid_missing_both,
...
ui <- page_navbar(
  ...
  nav_panel(
    title = "Data Quality",
    icon = bs_icon("shield-check-fill"),
    
    layout_columns(
      fill = FALSE, col_widths = c(2,2,2,2,2,2),
      value_box(title = "Valid Rows",          value = textOutput("valid_rows"),        showcase = bs_icon("check2-square"),           theme = "success"),
      value_box(title = "Missing Barcode",     value = textOutput("missing_barcode"),   showcase = bs_icon("upc-scan"),                theme = "warning"),
      value_box(title = "Missing Lab ID",      value = textOutput("missing_labid"),     showcase = bs_icon("person-x-fill"),           theme = "warning"),
      value_box(title = "Duplicate Pairs",     value = textOutput("duplicate_count"),   showcase = bs_icon("files"),                   theme = "warning"),
      value_box(title = "Barcode Conflicts",   value = textOutput("conflict_count"),    showcase = bs_icon("exclamation-triangle-fill"), theme = "danger"),
      value_box(title = "Avg Completeness",    value = textOutput("avg_completeness"),  showcase = bs_icon("graph-up"),                theme = "info")
    ),
    ...
    layout_columns(
      ...
      card(
        card_header(
          div(style = "display:flex; justify-content:space-between;",
              ...
              downloadButton("download_duplicates", "Download", class = "btn-sm"))
        ),
      ...
      card(
        card_header(
          div(style = "display:flex; justify-content:space-between;",
              span("Barcode Conflicts (Different Lab IDs)"),
              downloadButton("download_conflicts", "Download", class = "btn-sm"))
        ),
        DTOutput("conflicts_table")
      )
    ),
    ...
    card(
      card_header(
        div(style = "display:flex; justify-content:space-between;",
            ...
            downloadButton("download_completeness", "Download", class = "btn-sm"))
      ),
      DTOutput("completeness_table")
    ),
    
    card(
      card_header("Data Quality Rules"),
      card_body(
        tags$ul(
          tags$li(HTML("<strong>Valid Row:</strong> Must have both Barcode AND Lab ID (Numéro) filled")),
          tags$li(HTML("<strong>Duplicates:</strong> Same Barcode + Lab ID combination appearing multiple times")),
          tags$li(HTML("<strong>Conflicts:</strong> Same Barcode used with different Lab IDs")),
          tags$li(HTML("<strong>Completeness:</strong> Percentage of valid rows with non-empty values per column"))
        )
      )
    )
  ),
  ...
  nav_panel(
    ...
    layout_columns(
      ...
      value_box(
        ...
        showcase = bs_icon("people-fill", size = "2rem"),
        showcase_layout = "top right",
        theme = "success",
        height = "120px"
      ),
      value_box(
        ...
        showcase = bs_icon("geo-alt-fill", size = "2rem"),
        showcase_layout = "top right",
        theme = "warning",
        height = "120px"
      ),
      value_box(
        ...
        showcase = bs_icon("pin-map-fill", size = "2rem"),
        showcase_layout = "top right",
        theme = "danger",
        height = "120px"
      ),
  ...
  # === OTHER TABS ==========================================================
  nav_panel(title = "Geography", icon = bs_icon("map-fill"), mod_geo_map_ui("geo_map")),
  nav_panel(title = "Extraction QC", icon = bs_icon("droplet-half"), mod_extractions_qc_ui("extractions_qc")),
  nav_panel(title = "Lab Results", icon = bs_icon("clipboard2-data-fill"), mod_lab_results_ui("lab_results")),
...
server <- function(input, output, session) {
  ...
  # Data quality metrics - sidebar
  output$total_rows <- renderText({
    dq <- data_quality()
    if (is.null(dq)) return("—")
    scales::comma(dq$total_rows)
  })
  
  output$valid_rows <- renderText({
    dq <- data_quality()
    if (is.null(dq)) return("—")
    scales::comma(dq$valid_rows)
  })
  
  output$missing_barcode <- renderText({
    dq <- data_quality()
    if (is.null(dq)) return("—")
    scales::comma(dq$invalid_missing_barcode)
  })
  
  output$missing_labid <- renderText({
    dq <- data_quality()
    if (is.null(dq)) return("—")
    scales::comma(dq$invalid_missing_labid)
  })
  
  output$empty_rows <- renderText({
    dq <- data_quality()
    if (is.null(dq)) return("—")
    scales::comma(dq$empty_rows)
  })
  ...
  output$duplicate_count <- renderText({
    dq <- data_quality()
    if (is.null(dq)) return("—")
    scales::comma(nrow(dq$duplicates))
  })
  
  output$conflict_count <- renderText({
    dq <- data_quality()
    if (is.null(dq)) return("—")
    scales::comma(nrow(dq$barcode_conflicts))
  })
  
  output$avg_completeness <- renderText({
    dq <- data_quality()
    if (is.null(dq) || !nrow(dq$column_completeness)) return("—")
    avg_comp <- mean(dq$column_completeness$percent, na.rm = TRUE)
    sprintf("%.1f%%", avg_comp)
  })
  ...
  output$duplicates_table <- renderDT({
    dq <- data_quality()
    if (is.null(dq) || !nrow(dq$duplicates)) {
      return(datatable(tibble(Message = "No duplicates found"), 
                       options = list(dom = 't')))
    }
    datatable(dq$duplicates, 
              options = list(pageLength = 10, scrollX = TRUE),
              caption = "Rows with identical Barcode + Lab ID combinations")
  })
  
  output$conflicts_table <- renderDT({
    dq <- data_quality()
    if (is.null(dq) || !nrow(dq$barcode_conflicts)) {
      return(datatable(tibble(Message = "No conflicts found"), 
                       options = list(dom = 't')))
    }
    datatable(dq$barcode_conflicts,
              options = list(pageLength = 10, scrollX = TRUE),
              caption = "Barcodes associated with multiple Lab IDs")
  })
  
  output$completeness_table <- renderDT({
    dq <- data_quality()
    if (is.null(dq) || !nrow(dq$column_completeness)) {
      return(datatable(tibble(Message = "No data available"), 
                       options = list(dom = 't')))
    }
    
    dq$column_completeness %>%
      datatable(options = list(pageLength = 25, scrollX = TRUE),
                filter = "top") %>%
      formatStyle("percent",
                  backgroundColor = styleInterval(c(25, 50, 75),
                                                  c("#ffcccc", "#ffffcc", "#fff3cd", "#d4edda")))
  })
  
  # Download handlers for data quality
  output$download_duplicates <- downloadHandler(
    filename = function() sprintf("duplicates_%s.csv", format(Sys.Date(), "%Y%m%d")),
    content = function(file) {
      dq <- data_quality()
      if (!is.null(dq)) write.csv(dq$duplicates, file, row.names = FALSE)
    }
  )
  
  output$download_conflicts <- downloadHandler(
    filename = function() sprintf("barcode_conflicts_%s.csv", format(Sys.Date(), "%Y%m%d")),
    content = function(file) {
      dq <- data_quality()
      if (!is.null(dq)) write.csv(dq$barcode_conflicts, file, row.names = FALSE)
    }
  )
  
  output$download_completeness <- downloadHandler(
    filename = function() sprintf("column_completeness_%s.csv", format(Sys.Date(), "%Y%m%d")),
    content = function(file) {
      dq <- data_quality()
      if (!is.null(dq)) write.csv(dq$column_completeness, file, row.names = FALSE)
    }
  )
  ...
  observeEvent(input$load_data, {
    ...
    df_raw <- tryCatch({
      # Prefer skipping at read-time
      readxl::read_excel(
        input$selected_file,
        .name_repair = "minimal",
        col_select = -c(24:26)     # never read columns 24, 25, 26
      ) %>%
        janitor::clean_names() %>%
        mutate(across(everything(), as.character))
    }, error = function(e) {
      # Fallback: read then drop if older readxl doesn't support col_select
      tmp <- readxl::read_excel(input$selected_file, .name_repair = "minimal")
      if (ncol(tmp) >= 26) tmp <- tmp[, -c(24:26)]
      tmp %>%
        janitor::clean_names() %>%
        mutate(across(everything(), as.character))
    })
    ...
    if (!nrow(df_clean)) {
      showNotification("No valid rows (both Barcode and Lab ID required)", type = "warning")
      return()
    }
    ...
    showNotification(
      sprintf("✓ Loaded %s valid samples (from %s total rows)", 
  ...
  output$debug_data_info <- renderPrint({
    ...
    list(
      data_quality = list(
        ...
        valid_rows = dq$valid_rows,
        empty_rows = dq$empty_rows,
        missing_barcode = dq$invalid_missing_barcode,
        missing_labid = dq$invalid_missing_labid,
        duplicate_pairs = nrow(dq$duplicates),
        barcode_conflicts = nrow(dq$barcode_conflicts)
      ),
      clean_data = list(
        rows = nrow(df),
        columns = ncol(df),
