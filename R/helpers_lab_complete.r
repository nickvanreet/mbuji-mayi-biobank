...
#' @param pcr_cq_max Max Cq for PCR positivity
#' @param elisa_pp_cut PP% cutoff for ELISA positivity
#' @param ielisa_inh_cut % inhibition cutoff for iELISA positivity
#' @return Data frame with flags
#' @export
compute_flags <- function(df, pcr_cq_max = 38, elisa_pp_cut = 50, ielisa_inh_cut = 35) {
  df %>%
    dplyr::mutate(
      # PCR positive if any target below threshold
      PCR_pos = !is.na(Cq_177T) & (Cq_177T <= pcr_cq_max | Cq_18S2 <= pcr_cq_max),
    ...
    dplyr::select(-iELISA_pos_numeric, -iELISA_pos_text, -Tested_count, -Pos_count)
}
...
parse_pcr_controls <- function(dir) {
  # Placeholder - implement if control sheets exist
  tibble::tibble(
    File = character(),
